...
None outstanding. The most recent audit reconciled Sub-models 1, 5, 7 and 8 against everything built after them, including a genuine same-class field collision (`Alarm.source` vs. the Sub-model 15 addition, resolved as `Alarm.origin`) and a retired policy term (`on-demand` → `automaticMode: none` + universal `manualOverrideAvailable`).
